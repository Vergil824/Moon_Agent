...
import { Loader2, AlertCircle, RefreshCw, Smartphone, QrCode } from "lucide-react";
...
import { getPaymentChannel, type PaymentMethod, type PaymentChannelCode } from "@/lib/utils/utils";
...
export default function PaymentSubmitPage() {
  ...
  const submitPayment = useCallback(async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : "支付提交失败，请稍后重试";
      setErrorMessage(message);
      setStatus("error");
      setErrorDialogOpen(true);
    }
  }, [payOrderId, method, channelCode, session?.accessToken, returnUrl, openid]);
