...
public class AfterSaleServiceImpl implements AfterSaleService {
    ...
    private AfterSaleDO validateAfterSaleReceivable(Long id) {
        ...
        // 支持状态20（商品待退货）和状态30（待卖家收货），便于测试环境跳过物流填写
        if (!ObjectUtil.equal(afterSale.getStatus(), AfterSaleStatusEnum.SELLER_AGREE.getStatus()) 
                && !ObjectUtil.equal(afterSale.getStatus(), AfterSaleStatusEnum.BUYER_DELIVERY.getStatus())) {
            throw exception(AFTER_SALE_CONFIRM_FAIL_STATUS_NOT_BUYER_DELIVERY);
        }
